...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $clinicPassword = trim($_POST['clinic_password']); 
    ...
    if (isset($_FILES['proof_files']) && count($_FILES['proof_files']['name']) > 0) {
        ...
        foreach ($_FILES['proof_files']['name'] as $key => $name) {
            if ($_FILES['proof_files']['error'][$key] == 0 && in_array($_FILES['proof_files']['type'][$key], $allowedTypes)) {
    ...
    if (empty($clinicName) || empty($clinicOwner) || empty($clinicEmail) || empty($clinicPhone) || empty($clinicAddress) || empty($clinicPassword)) {
        ...
        $stmt = $conn->prepare("INSERT INTO clinic (clinicName, clinicOwner, email, password, cNumber, offeredServices, clinicAddress, proof, clinicPhoto) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("sssssssss", $clinicName, $clinicOwner, $clinicEmail, $clinicPassword, $clinicPhone, $offeredServices, $clinicAddress, $proofFile, $clinicPhoto);
        ...
        if ($stmt->execute()) {
            ...
            foreach ($days as $day) {
                if (isset($_POST[strtolower($day) . '_enabled'])) {
                    ...
                    if (!$scheduleStmt->execute()) $scheduleInserted = false;
                    $scheduleStmt->close();
                } else {
            ...
            $message = $scheduleInserted ? 'Clinic added successfully!' : 'Clinic added but there was an issue with the schedule.';
            $messageType = $scheduleInserted ? 'success' : 'warning';
            $_POST = array();
        } else {
            $message = ($conn->errno == 1062) ? 'Clinic name or email already exists.' : 'Error adding clinic: ' . $conn->error;
            $messageType = 'error';
        }
        $stmt->close();
    }
...
<div class="flex flex-col p-5 bg-white w-full rounded-lg shadow-sm">
        ...
        <div class="grid grid-cols-2 gap-x-5 gap-y-2 mt-5">
            ...
            <div>
                <input type="text" name="clinic_name" placeholder="Clinic name" value="<?php echo htmlspecialchars($_POST['clinic_name'] ?? ''); ?>" required class="w-full px-2 py-2 border border-lightgrey-100 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 text-offblack-100 text-sm placeholder:text-sm">
            </div>
            <div>
                <input type="text" name="owner_name" placeholder="Owner name" value="<?php echo htmlspecialchars($_POST['owner_name'] ?? ''); ?>" required class="w-full px-2 py-2 border border-lightgrey-100 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 text-offblack-100 text-sm placeholder:text-sm">
            </div>

            <div class="text-offblack-100 font-semibold">Clinic Email *</div>
            <div class="text-offblack-100 font-semibold">Clinic Password *</div>

            <div>
                <input type="email" name="clinic_email" placeholder="Clinic email" value="<?php echo htmlspecialchars($_POST['clinic_email'] ?? ''); ?>" required class="w-full px-2 py-2 border border-lightgrey-100 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 text-offblack-100 text-sm placeholder:text-sm">
            </div>
            <div>
                <input type="password" name="clinic_password" placeholder="Clinic password" value="<?php echo htmlspecialchars($_POST['clinic_password'] ?? ''); ?>" required class="w-full px-2 py-2 border border-lightgrey-100 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 text-offblack-100 text-sm placeholder:text-sm">
            </div>
            ...
            <div>
                <textarea name="offered_services" cols="10" rows="5" placeholder="Enter offered services..." class="resize-none w-full px-2 py-2 border border-lightgrey-100 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 text-offblack-100 text-sm placeholder:text-sm"><?php echo htmlspecialchars($_POST['offered_services'] ?? ''); ?></textarea>
                <strong>Tip:</strong> Press Enter or start a new line to automatically add each service
            </div>
              <div>
                <input type="text" name="clinic_phone" placeholder="Clinic phone number" value="<?php echo htmlspecialchars($_POST['clinic_phone'] ?? ''); ?>" required class="w-full px-2 py-2 border border-lightgrey-100 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 text-offblack-100 text-sm placeholder:text-sm">
            </div>
        </div>
    </div>
